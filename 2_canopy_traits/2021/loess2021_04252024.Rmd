...
txt_files_ls <- list.files("/Users/jcooper/Desktop/thesis_research/canopy_cover/1_image_masking/image_analysis/data_analysis/2021_doubledilated/canopy_closure/",pattern = "txt",full.names = T, recursive = TRUE)
...
new_names <- c(258, 413.5, 619, 765.5, 874, 1096, 1190, 1251, 1421.5, 1759.5)

# Rename columns to GDD 
for(i in 1:10) names(combined_df)[names(combined_df) == old_names[i]] = new_names[i]

# Save as .csv
write.csv(combined_df,"/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/data_processing/1PlantRatio.csv", row.names = FALSE)
...
plot_roi <- read.csv("/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/plot_order/Plots_widiv_ROI2021.csv")

# read in list of plots
plot_list <- as.matrix(read.csv("/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/plot_order/Plots_only_WIDIV.csv"))

# Path of location to save the plot orders
plot_order_loc = "/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/plot_order/"
...
CC_final <- read.csv("/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/data_processing/1PlantRatio.csv", check.names = FALSE)

# Read in plot order file
p_order <- read.table("/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/plot_order/Plots_WIDIV_Ordered.txt", header = TRUE)
...
write.csv(CC_final2, "/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/data_processing/2PlantRatio_plot.csv",  row.names = FALSE)
...
c2021 <- read.csv("/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/data_processing/2PlantRatio_plot.csv", check.names = FALSE)

# Read in stand counts
stand2021 <- read.csv("/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/field_data/2021_stand_JC01052021.csv")

# Merge by "Plot"
c_stand <- merge(c2021, stand2021, by = "Plot")
...
c_standfilter <- c_stand %>%
  filter(Stand > 14) # n = 860

# Save data frame
write.csv(c_standfilter, "/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/data_processing/3PlantRatio_plot_stand.csv",  row.names = FALSE)

##################
# QC

# Pivot Table into "long"
ccfinal_long <- pivot_longer(c_standfilter, cols=2:11, names_to="flight", values_to="canopy_cover")
...
ggplot(data=ccfinal_long, aes(x=flight, y=canopy_cover)) +
  ...
  xlab('Flight (GDD)') + 
  ylab('Plant Ratio') + 
  ggtitle("2021 Double Dilated Percent Canopy Cover - Weeds, Stand") +
  labs(caption = "n = 860")
...
# Read in data
c2021 <- read.csv("/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/data_processing/3PlantRatio_plot_stand.csv", check.names = FALSE)
...
filter_peaks <- function(df) { #Input df into function
  ...
  c2021_dipdrop <<- big_data # Export data frame with peaks and valleys labeled outside of function
...
c2021_dipdrop$closureNA <- c2021_dipdrop$closure

# Replace data points with NA if tagged as peak or valley
c2021_dipdrop$closureNA <- ifelse(is.na(c2021_dipdrop$...4), c2021_dipdrop$closure, ifelse(c2021_dipdrop$...4 == "dip",  NA, ifelse(c2021_dipdrop$...4 == "peak", NA, c2021_dipdrop$closure)))

# Remove extra columns
c2021_dipdrop <- c2021_dipdrop[,c(1, 2, 5)]

# Pivot wide To count # of NA per row
c2021_wide <- pivot_wider(c2021_dipdrop, names_from = "flight", values_from = "closureNA")

# Make new column with na counts
c2021_wide$na_count <- apply(c2021_wide, 1, function(x) sum(is.na(x)))

# Pivot back to long to filter out na counts past certain threshold
c2021_long <- c2021_wide %>%
  pivot_longer(cols = 2:11, names_to = "flight", values_to = "closure")

# Count na
c2021_long %>%
  summarise(sum(is.na(closure))) # 384 NA data points

# Filter plots with na count > 3
c2021_filter <- c2021_long %>%
  filter(na_count < 3)

# Count number of plots
c2021_filter %>%
  summarise(n_distinct(Plot)) # Keeps 827 of 859 plots

# Count na
c2021_filter%>%
  summarise(sum(is.na(closure))) # 275 NA data points

# Pivot back to wide
c2021_final <- pivot_wider(c2021_filter, names_from = "flight", values_from = "closure")

# Save data frame
write.csv(c2021_final, "/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/data_processing/4PlantRatio_plot_stand_dips.csv",  row.names = FALSE)

##################
# QC

# Make "flight" numeric
c2021_filter$flight <- as.numeric(c2021_filter$flight)

# Plot
ggplot(data=c2021_filter, aes(x=flight, y=closure)) +
  #geom_point(color="gray", size=.1) +
  geom_line(aes(group=Plot), color="gray") +
  stat_smooth(method="loess", formula = y ~ x) +
  ...
  xlab('Flight (GDD)') + 
  ylab('Plant Ratio') + 
  ggtitle("2021 Double Dilated Percent Canopy Cover - Weeds, Stand, Peaks and Valleys") +
  labs(caption = "n = 827")
...
C <- read.csv("/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/data_processing/4PlantRatio_plot_stand_dips.csv", check.names = FALSE)

# Drop na_count column
C <- C[, c(1, 3:12)]


C_long <- pivot_longer(C, cols = 2:11, names_to = "flight", values_to = "canopy_cover") #Pivot long based on flight columns
...
Loess_Plot_Predictions2021 <- y3 # Rename data frame
Loess_Plot_Predictions2021$Plot <- Loess_Plot_Predictions2021$rowname # Copt row name column with new name
Loess_Plot_Predictions2021$rowname <- NULL # Remove old row name column

Loess_Plot_Predictions2021 <- Loess_Plot_Predictions2021[, c(29,1:28)] #Reorganize so GDD is first column, canopy_cover is second

# Save data frame
write.csv(Loess_Plot_Predictions2021, "/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/data_processing/5PlantRatio_loess.csv",  row.names = FALSE)
...
# Load loess
loess <- read.csv("/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/data_processing/5PlantRatio_loess.csv", check.names = FALSE)

# Load experimental design info
design <- read.csv("/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/data/field_data/2021_Widiv_FieldBook.csv")

# Merge data frames
design_loess <- merge(loess, design, by = "Plot")

write.csv(design_loess, "/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/results/2021_loess_CC.csv",  row.names = FALSE)
...
p <- ggplot(data=c_long, aes(x=flight, y=closure)) +
  ...
  xlab('Flight (GDD)') + 
  ylab('Plant Ratio') + 
  ggtitle("2021 Double Dilated Percent Canopy Cover - Loess") +
  labs(caption = "n = 839, span = .3")

p

ggsave("/Users/jcooper/Desktop/thesis_research/canopy_cover/2_canopy_traits/2021/results/2021_loess_CC.png", p, width=4, height=4)
